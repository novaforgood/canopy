...
CREATE OR REPLACE VIEW "public"."past_day_unread_messages_count" AS 
 SELECT chat_message.sender_profile_id,
  ...
  WHERE ((chat_message.created_at > (now() - '1 hour'::interval)) AND (chat_message.sender_profile_id <> ptcr.profile_id) AND (chat_message.id > COALESCE(ptcr.latest_read_chat_message_id, (0)::bigint)) AND (sender_user.type = 'User'::text))
...
CREATE OR REPLACE VIEW "public"."past_day_unread_messages_count" AS 
 SELECT chat_message.sender_profile_id,
   ...
   FROM ((((chat_message
     ...
     JOIN profile sender_profile ON ((sender_profile.id = chat_message.sender_profile_id)))
     JOIN "user" sender_user ON ((sender_user.id = sender_profile.user_id)))
  WHERE ((chat_message.created_at > (now() - '1 day'::interval)) AND (chat_message.sender_profile_id <> ptcr.profile_id) AND (chat_message.id > COALESCE(ptcr.latest_read_chat_message_id, (0)::bigint)) AND (sender_user.type = 'User'::text))
  GROUP BY ptcr.profile_id, chat_message.sender_profile_id;
