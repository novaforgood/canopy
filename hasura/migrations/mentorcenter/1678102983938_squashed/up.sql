...
CREATE OR REPLACE VIEW "public"."past_day_unread_messages_count" AS 
 SELECT chat_message.sender_profile_id,
  ...
  WHERE chat_message.is_system_message = false
  AND ((chat_message.created_at > (now() - '1 day'::interval)) 
  AND (chat_message.sender_profile_id <> ptcr.profile_id) 
  AND (chat_message.id > COALESCE(ptcr.latest_read_chat_message_id, (0)::bigint)) AND (sender_user.type = 'User'::text))
